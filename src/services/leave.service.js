...
// Leave request types with balances for an employee and company
async function getLeaveRequestTypes(EmpID, CompanyID, Year = (new Date()).getFullYear()) {
  const pool = await sql.connect(dbConfig);
  const res = await pool.request()
    .input('EmpID', sql.VarChar(30), EmpID)
    .input('CompanyID', sql.VarChar(30), CompanyID)
    ...
    .query(`
      SELECT
        lt.type_code AS TypeCode,
        lt.type_name AS TypeName,
        (lt.annual_quota - ISNULL(ulb.used_days, 0)) AS RemainingDays
      FROM LeaveTypes lt
      LEFT JOIN UserLeaveBalance ulb
        ON ulb.type_code = lt.type_code
       AND ulb.empID = @EmpID
       AND ulb.companyID = @CompanyID
       AND ulb.year = @Year
...
async function cancelLeave(LeaveReqID) {
  ...
  await pool.request()
    .input('reqID', sql.VarChar(30), LeaveReqID)
    .query('UPDATE LeaveReqTable SET status = \'Draft\' WHERE reqID = @reqID');
}
...
async function getLeaveRequestDetails(reqId) {
  return getLeaveById(reqId);
}
