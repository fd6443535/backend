...
async function cancelDocumentRequest(DocumentReqID) {
  const pool = await sql.connect(dbConfig);
  await pool.request()
    .input('DocumentReqID', sql.VarChar(30), DocumentReqID)
    .query('UPDATE DocumentReqTable SET Status=\'Draft\' WHERE documentReqID=@DocumentReqID');
}

// Document request types master (code + name)
async function getDocumentRequestTypes() {
  const pool = await sql.connect(dbConfig);
  const result = await pool.request()
    .query(`
