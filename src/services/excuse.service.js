...
// New: Cancel excuse request by setting status to 'Draft'
async function cancelExcuseRequest(ExcuseReqID) {
  const pool = await sql.connect(dbConfig);
  await pool.request()
    .input('excuseReqID', sql.VarChar(30), ExcuseReqID)
    .query(`UPDATE ExcuseReqTable SET status='Draft' WHERE excuseReqID=@excuseReqID`);
}
