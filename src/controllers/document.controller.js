...
// Get document request types (code + name)
exports.getDocumentRequestTypes = async (req, res) => {
  try {
    const types = await documentService.getDocumentRequestTypes();
    res.json(types);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// Cancel a document request by setting Status to 'Draft'
exports.cancelDocumentRequest = async (req, res) => {
  try {
    const reqID = req.params.reqid || req.query.reqid;
    await documentService.cancelDocumentRequest(reqID);
    res.json({ message: 'Document request set to Draft' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
exports.getPendingDocumentRequestDetails = async (req, res) => {
  try {
    const requestId = req.params.reqid || req.query.reqid;
