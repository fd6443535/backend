...
// Cancel an excuse request by setting status to 'Draft'
exports.cancelExcuseRequest = async (req, res) => {
  try {
    const excuseId = req.params.reqid || req.query.reqid;
    await excuseService.cancelExcuseRequest(excuseId);
    res.json({ message: 'Excuse request set to Draft' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
