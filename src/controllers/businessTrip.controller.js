...
exports.editBusinessTripRequest = async (req, res) => {
  try {
    const { reqid: tripId, ...updateData } = req.body;
...
exports.draftSaveBusinessTripRequest = async (req, res) => {
  try {
    const data = req.body;
    const file = req.file;
    // Ensure EmpID is provided to the service; frontend cannot read httpOnly cookies
    if (!data.EmpID) data.EmpID = req.cookies.empid;
...
exports.delegateBusinessTripRequest = async (req, res) => {
  try {
    const { reqid: reqID, newApproverEmpID, comments } = req.body;
...
// Cancel a business trip request (set status to 'Draft')
exports.cancelBusinessTripRequest = async (req, res) => {
  try {
    const tripId = req.params.reqid || req.query.reqid;
    await businessTripService.cancelBusinessTripRequest(tripId);
    res.json({ message: 'Business trip request set to Draft' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
exports.approveRejectBusinessTripRequest = async (req, res) => {
  try {
    const { reqid: tripId, action, comments } = req.body;
