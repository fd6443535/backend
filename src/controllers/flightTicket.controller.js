...
// Cancel a flight ticket request (set status to 'Draft')
exports.cancelFlightTicketRequest = async (req, res) => {
  try {
    const requestId = req.params.reqid || req.query.reqid;
    await flightTicketService.cancelFlightTicketRequest(
      requestId,
      req.cookies.context.companyid
    );
    res.json({ message: 'Flight ticket request set to Draft' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
