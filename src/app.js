...
// Express 5 uses path-to-regexp v6 which doesn't accept '*' as a path; use '/(.*)'
...
app.get('/web/endpoints', (req, res) => {
  try {
    ...
    const ensureReqIdHeaderHeuristic = (pathOnly, method, originalHeaders) => {
      ...
      // If the path or query already carries a reqid segment or query param, do not add header
      if (/\breqid\b/i.test(p)) return false;
    ...
    // Normalize known path discrepancies between Postman collection and backend routes
    const rewritePath = (moduleName, method, pathOnly) => {
      try {
        if (typeof pathOnly !== 'string') return pathOnly;
        let p = pathOnly;
        if ((moduleName || '').toLowerCase() === 'leave') {
          // Align Leave endpoints with latest backend routes
          p = p.replace('/api/leave/getPendingLeaves', '/api/leave/getPendingLeaveRequests');
          p = p.replace('/api/leave/getLeavesById', '/api/leave/getLeaveById');
          p = p.replace('/api/leave/applyLeave', '/api/leave/submitLeaveRequest');
          // Map pending details to the unified details endpoint
          if (p.endsWith('/api/leave/getPendingLeaveRequestDetails') || p.includes('/api/leave/getPendingLeaveRequestDetails/')) {
            p = p.replace('/api/leave/getPendingLeaveRequestDetails', '/api/leave/getLeaveRequestDetails');
          }
          // Convert any trailing /{{reqid}} to query param for details endpoint
          if (/^\/api\/leave\/getLeaveRequestDetails\/.+/.test(p)) {
            p = p.replace(/^\/api\/leave\/getLeaveRequestDetails\/.+$/, '/api/leave/getLeaveRequestDetails?reqid={{reqid}}');
          }
          if (method === 'GET' && p.endsWith('/api/leave/getLeaveRequestDetails') && !p.includes('?')) {
            p = p + '?reqid={{reqid}}';
          }
        }
        return p;
    ...
    for (const mod of topItems) {
      ...
      for (const ep of subItems) {
        ...
        const finalPath = rewritePath(moduleName, method, pathOnly);
        const normalizedUrl = '{{baseUrl}}' + finalPath;
        const predefined = {
          headers: normalizeHeaders(headers, finalPath, method),
          body: normalizeBody(body)
        };
        const isObsoleteLeaveHistory = (moduleName || '').toLowerCase() === 'leave' && /\/api\/leave\/getLeaveHistory$/i.test(finalPath);
        if (!isObsoleteLeaveHistory) {
          endpoints.push({ name: ep.name || '', method, url: normalizedUrl, headers, body, predefined });
        }
      }
